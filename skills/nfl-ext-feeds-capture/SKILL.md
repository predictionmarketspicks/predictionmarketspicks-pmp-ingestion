...
1. **Chrome logins.** Confirm Benny's Chrome (the one Claude-in-Chrome drives) is logged into **`premium.pff.com`** AND **`ftnfantasy.com`**. Open each site's home once and check for the signed-in state. **A logged-out PFF blocks ALL THREE PFF feeds — there is no free fallback** (corrected 2026-08-03; the earlier "free PFF pages work logged-out" line was wrong and cost a run). Verified while logged out: `premium.pff.com/nfl/teams/…` renders 32 team rows with **every grade column blank**; `pff.com/nfl/grades/position/QB` shows grades for only the **top 3** players (rest bio-only); `pff.com/betting/nfl-power-rankings` caps at **10 of 32** behind "Unlock … with PFF+". All three are the dangerous half-capture case — the row count still looks plausible. **Alert (§7) and skip; never ingest a logged-out PFF page.** ⛔ **FTN is the same trap, not the exception** (corrected 2026-09-21 — this line used to say FTN "renders complete and is unaffected", and Run A that day nearly ingested it): logged out, `ftnfantasy.com/stats/nfl/team-total-dvoa` still renders **all 32 rows and all 27 columns**; `team`, `total_dvoa`, `total_dvoa_rank`, `week`, `year` carry values and the other 22 cells are **empty strings**. 25 of `ext_team_dvoa`'s 30 columns accept NULL, so that upserts cleanly and every count-based check passes. The logged-out tell is the header carrying `Log In` / `Sign Up` with the grade cells blank — assert it before reading anything. The runner's null-density gate (§6.6) now refuses this shape; it is the belt, the login check is the suspenders.
...
- `grades-player` ← `GET /v1/facet/{passing,rushing,receiving,field_goal}/summary?league=nfl&season=$SEASON`, filtered client-side to `{QB}` / `{HB,RB}` / `{WR,TE}` / `{K}` respectively (the facets themselves return every position that logged a snap in that phase — e.g. `rushing` includes O-linemen on a fumble return — so the position filter is required, not optional). Grades map the same way as team-level. Everything else in the row (yards, EPA, pressure rate, drop rate, …) rides into `extra{}` unchanged, which is what actually backs `ext_player_grades` today.
  - ✅ **Position scope — DECIDED 2026-09-21 (Benny asked; answer stands): leave the `FACETS` filter where it is.** `capture-pff-api.js` `FACETS` is the only thing imposing the skill-position set; `src/feeds/grades-player.js` has no position filter. Season 2026 week 1 therefore holds **1,228** rows from two differently-scoped captures — 895 written 09-18 across 15 positions (DI, ED, CB, S, LB, T, G, C, FB, P, LS…) and 332 written 09-21 across the skill positions. Nothing was overwritten (upsert key `season, week, player_id, position`; the sets are complementary), so a later row count reading "1,228 ≠ hundreds-from-one-capture" is expected, not a fault. Audited both repos: **nothing reads `ext_player_grades`** — only the writer, the freshness workflow, and the site repo's lint rule that bans the name. The table also has **no defensive grade columns**, so a DI/ED/CB/S/LB/K row carries `off`/`pass`/`run`/`recv` all null by construction (648 of the 1,228) — the null-density gate scopes its grades-player rule to offensive positions for exactly that reason. If defense is ever wanted in the model, the cheaper lever is `ext_team_grades`, which already stores `pblk, rblk, prsh, cov, rdef, tack, spec` from the same `/v1/teams/overview` call and **no reader selects any of them** (`run_consensus.py:73` takes `team,overall`; `backtest_consensus.py:91` takes `team,overall,pf,pa`). Separate spec if wanted; don't widen the player capture against the shared 100-reads/min budget.
...
| **`power-ranks`** → `power-ranks.json` | `pff.com/betting/nfl-power-rankings` — **the CSV, never the rendered table**: Recipe A below reads the CSV Blob in-page (works from Cowork); the download-to-`~/Downloads` path only works from Claude Code on the Mac | **PFF premium** (logged out = top 10 only) | **`week`: N** (0 = preseason) | **32 rows** | team, point_spread_rating, qb_rating, sos_to_date, sos_remaining, sim_avg_wins, make_playoffs_pct, win_division_pct, win_conference_pct, win_super_bowl_pct |
| **`dvoa-team`** → `dvoa-team.json` | `ftnfantasy.com/stats/nfl/team-total-dvoa` — **there is NO export button** (MUI DataGrid, no `<table>`, SSR, no data API): read the grid with Recipe B below | FTN login (logged out = all 32 rows with blank grade cells, §2.1) | **`week`: N** | **32 rows** | team, tot_dvoa(+rank), non_adj_voa, wins, losses, last_year_rank, off_dvoa(+rank), def_dvoa(+rank), st_dvoa(+rank), off_voa, def_voa, st_voa, est_wins(+rank), wei_dvoa(+rank), sched_past(+rank), sched_future(+rank), variance(+rank) |
...
- ✅ **STANDING AUTHORIZATION (Benny, 2026-09-02): downloading the CSV export from `pff.com/betting/nfl-power-rankings` is PRE-APPROVED for every Run A / Run B fire.** Treat it as the user having said yes in chat — do not stop to ask, do not skip the feed for lack of sign-off. Scope is exactly that one page's CSV button; it is public power-rankings data. The 2026-09-02 Run B skipped power-ranks solely because this line did not exist, and a second miss would have tripped the in-season freshness alert on ~09-08. Any OTHER download still needs sign-off. ⚠️ **Which context you are in decides HOW you get the CSV** (added 2026-09-21): the button writes the file to `~/Downloads`, which **Claude Code on the Mac can read and Cowork cannot** — Cowork's sandbox mounts only `prediction-marketspicks`, `pmp-ingestion`, outputs and uploads. From Cowork, use **Recipe A** below, which reads the CSV Blob in-page and never touches the filesystem. Falling back to the rendered table is the strict downgrade described next — don't.
...
**DOM extraction recipes (added 2026-09-21, both verified on Run A that day).** Run these through Claude-in-Chrome's `javascript_tool` on the already-open, logged-in page. Neither writes a file.

- **Recipe A — PFF power-ranks CSV without a download.** The CSV button builds a Blob client-side. Hook `URL.createObjectURL`, swallow the anchor's download click, click the button, read the Blob as text. Full precision, no `~/Downloads` dependency. Verified 2026-09-21: ARZ `win_super_bowl_pct` came back `0.07` where the rendered table shows `<1%`.
...
- **Recipe B — FTN DVOA from the MUI DataGrid.** No export, no `<table>`, no data request on the network (SSR; only analytics). Every cell renders per row with a stable `data-field`:
  ...
  `data-field` names, in grid order → staging keys: `team`→team · `total_dvoa`→tot_dvoa · `total_dvoa_rank`→tot_dvoa_rank · `non_adj_tot_voi`→non_adj_voa · `w_l`→wins/losses (split on `-`) · `last_week`→last_year_rank · `offense_dvoa`/`offense_rank` · `defense_dvoa`/`defense_rank` · `special_teams_dvoa`/`special_teams_rank` · `offense_voa_unadj`→off_voa · `defense_voa_unadj`→def_voa · `special_voa_unadj`→st_voa · `estim_wins`→est_wins · `rank1`→est_wins_rank · `wei_dvoa` · `rank2`→wei_dvoa_rank · `past_schedule`→sched_past · `rank3`→sched_past_rank · `future_schedule`→sched_future · `rank4`→sched_future_rank · `var`→variance · `rank5`→variance_rank · `week` · `year`.

  Two gotchas: **`last_week` is FTN's LAST YEAR column** and renders `x` for every team in week 1 → omit `last_year_rank`, never write `0` (the null-density gate deliberately does not list it as required). And the **logged-out tell** is the header carrying `Log In` / `Sign Up` with the grade cells empty (§2.1) — cheap to assert before reading.

- **Operator note — `javascript_tool` truncates its return at roughly 1,000–1,400 characters.** A 32×27 JSON blob loses its tail silently. Emit large extractions as pipe-delimited chunks of ~8 rows per call, or the last teams never reach the staging file and the null-density gate is what tells you.
...
6. **Check the EXIT CODE, not just the printed counts** (added 2026-08-03). The runner now distinguishes three outcomes, and only one of them is silent:
   ...
   - `BLOCKED` — stale file, `--season` disagreeing with the file, or (since 2026-09-21) **a required column mostly null** (`<feed>: <col> null on N/M rows`). **Nothing written, exit 1.** Never wave this through with `--allow-stale` / `--allow-sparse="<reason>"` to "make it pass" — it is the guard doing its job.
   - **Null density (the third guard, `handoffs/NFL_EXT_FEEDS_HALF_CAPTURE_GATE_2026-09-21.md`).** A paywall blanks cells, it does not thin rows — logged-out FTN is 32/32 with 22 empty columns, and every count check goes green. The runner checks a per-feed `REQUIRED_COLUMNS` map after normalization (`dvoa-team`: off/def/st_dvoa, est_wins, wei_dvoa · `power-ranks`: point_spread_rating, sim_avg_wins, make_playoffs_pct · `grades-team`: overall, off, def · `grades-player`: ≥1 of off/pass/run/recv per **offensive-position** row) and BLOCKS at >10% null. The threshold is not zero on purpose: FTN's LAST YEAR column is `x` on all 32 rows in week 1 and that is correct. Prove the gate can see a blank column before trusting a run: `node scripts/ingest-ext-feeds.js --self-test` → `SELF-TEST PASS (18 checks)`.
